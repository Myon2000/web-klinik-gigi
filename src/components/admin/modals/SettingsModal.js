...
import { Settings, X, Lock, Database, Clock, ShieldCheck, AlertTriangle, Trash2, KeyRound } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 max-w-lg w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        {/* Navigation Tabs */}
        <div className="flex border-b border-slate-200 bg-slate-50/70 px-4 pt-2 gap-1 text-xs">
          <button
            type="button"
            onClick={() => setActiveTab("operational")}
            className={`flex items-center gap-1.5 px-3.5 py-2.5 font-semibold rounded-t-lg border-b-2 transition-colors cursor-pointer ${
